...
void solve()
{
    ...
    for(int t=0;t<q;++t){
        int k;cin>>k;
        vector<int> data(k);
        for(int i=0;i<k;++i){cin>>data[i];--data[i];}
        if(k<3){
            cout<<"YES\n";
            continue;
        }
        u=data[0],v=data[1];
        int x,y;
        int p=lca(u,v,depth,parent,level);
        bool found =  true;
        for(int j=2;j<k;++j){
            x=lca(u,data[j],depth,parent,level);
            y=lca(v,data[j],depth,parent,level);
            if(p==u && x==data[j] && y==data[j]){
                u=data[j];
            }
            else if(p==v && x==data[j] && y==data[j]){
                v=data[j];
            }
            else if((x==data[j] && y==p) || (x==p && y==data[j]));
            else if(x==u && y==p)u=data[j];
            else if(y==v && x==p)v=data[j];
            else {
                found = false;
                break;
            }
